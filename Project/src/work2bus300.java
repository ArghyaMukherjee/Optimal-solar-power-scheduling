...
public class work2bus300 {

	/* Static variable declarations */
	
	static int noOfDays = 15, noOfLocations = 50, noOfSlots = 4, noOfBuses = 300, noOfBranches = 411, loadBusCount = 231, genBusCount = 68, colcount = 50, vertices = 302;
	...
	// Total demand in a basic IEEE 300 bus system without solar power injection is 23527.04 MW
	
		static double  demand = 23527.04;
		
	// Declare path for grid data file
	
	static String gridPath = "C:\\Users\\Arghya\\Documents\\BTP\\Project_Anoop\\Project_Anoop\\input files\\bus data\\loadflowdata300.xls";
			...
			// Process all other rows				
				...
				while(cellit.hasNext()){
					...
					// Store ground admittance of each transmission line
						...
						if(admittance[rcount] < 0)
							admittance[rcount] = -admittance[rcount];
						if(admittance[rcount] == 0)
							admittance[rcount] = -.1;
						ccount++;
					}
...
//		Matrix matInverse = Ybus.inverse();
	...
	//	inverse = matInverse.getArray();
		...
		int rowindex = 1;		
		try{
			BufferedReader breader = new BufferedReader(new FileReader("C:\\Users\\anoop\\Dropbox\\project\\loadflowmatlab\\inverseoutput.txt"));
			String tempLine;
			int lineCount = 1;
			while((tempLine = breader.readLine()) != null){
				inverse[rowindex - 1][lineCount - 1] = Double.parseDouble(tempLine);
				lineCount++;
				if(lineCount == noOfBuses + 1){
					lineCount = 1;
					rowindex++;
				}
			}
		...
		{
			rcost[j]= ThreadLocalRandom.current().nextDouble(5, 10);
		}
		
		
		for (int j=0; j< noOfBuses; j++ )
		{
			nrcost[j]= ThreadLocalRandom.current().nextDouble(500, 1500);
		}
		...
		for(int i = 0;i < noOfBuses;i++){
 		...
 		try{
			...
			for(int i = 0;i < noOfSlots;i++){
				...
				createLPModel(cplex, forecast[i], sd[i],rcost, nrcost, inverse, pgiMax, loadBus, genBus, Pli);
				...
				solveAndPrint(cplex, selectLocations[i], nrGeneration);
				...
				for(int j = 0;j < noOfLocations;j++){
					if(selectLocations[i][j] > 0){
						Ping[(int)loadBus[j] - 1] += committed[i][j];
					}
				}
				...
				System.out.println("Committed vs actual values\n");
				for(int j = 0;j < noOfLocations;j++)
					System.out.print("Location "+ j + "\t" + committed[i][j] + "\t" + actual[i][j] + "\n");
				System.out.println();
		...
		for(int i = 0;i < noOfSlots;i++){
			...
			for(int j = 0;j < noOfLocations;j++){				
				...
				// If location has been selected
					...
					if(actual[i][j] >= committed[i][j]){
						...
						newPgi[(int)loadBus[j] - 1] += committed[i][j];
					}
					...
					// If renewable generation is less than committed generation
						...
						// If available storage at location j is not enough to compensate deficiency
							...
							newPgi[(int)loadBus[j] - 1] += storage[j];
			...
			if(flowFlag != 0){				
				...
				*/
				...
				newPli[79] -= totalDef / 2;
				newPli[81] -= totalDef / 2;
				...
				*/
				...
				for(int j = 0;j < noOfLocations;j++){
					if(storage[j] > 0){
						capacity[0][(int)loadBus[j]] = storage[j];
						adjacency[0].add((int)loadBus[j]);
					}					
				}
				/* add edges to sink vertex */
				adjacency[79].add(vertices - 1);
				capacity[79][vertices - 1] = totalDef / 2;
				adjacency[81].add(vertices - 1);
				capacity[81][vertices - 1] = totalDef / 2;
				...
				while(true){
					...
					if(parent[vertices - 1] == -1){
						...
						/*update storage*/
						for(int j = 0;j < noOfLocations;j++){
							if(flow[source][(int)loadBus[j]] > 0){
								storage[j] -= flow[source][(int)loadBus[j]];
							}
						}
	...
	static void solveAndPrint(IloCplex cplex, double[] selectLocations, double[] nrGeneration){
		...
		{			
			...
			if(cplex.solve()){
				...
				fileWriter = new FileWriter("Output300.txt");
	...
	static void createLPModel(IloMPModeler model, double[]forecast, double[] sd, double[] rcost, double [] nrcost,  double[][] inverse, double[] pgiMax, double[] loadBus, double[] genBus, double[] Pli){
