...
public class work1bus14 {

	/* Static variable declarations */
	
	static int noOfDays = 15, noOfLocations = 9, noOfSlots = 4, noOfBuses = 14, noOfBranches = 20, loadBusCount = 9, genBusCount = 5, selected = 2, colcount = 9;
	...
	// C:\Users\Arghya\Documents\BTP\data\data
	...
	public static void main(String[] args) {
		...
		System.out.println("Renewable costs:\n");
		
		for (int j=0; j< colcount; j++ )
		{
			rcost[j]= ThreadLocalRandom.current().nextDouble(1, 10);
			System.out.println(j+ "\t" + rcost[j]);
		}
		
		System.out.println("Non-renewable costs:\n");
		for (int j=0; j< noOfBuses; j++ )
		{
			nrcost[j]= ThreadLocalRandom.current().nextDouble(500, 1500);
			System.out.println(j+ "\t" + nrcost[j] );
		}
	...
	static void solveAndPrint(IloCplex cplex){
		...
		{			
			...
			if(cplex.solve()){
		        ...
		        System.out.println(" Locations commiting solar power in this slot = " + numsolar);
		    }
	...
	static void createLPModel(IloMPModeler model, double[]forecast, double[] sd, double[] rcost, double[] nrcost, double[][] inverse, double[] pgiMax, double[] loadBus, double[] genBus, double[] Pli){
		...
		/*	IloLinearNumExpr constraintExpr = model.linearNumExpr();		
			...
			*/
		    ...
		    System.out.println("power forecast vs committed values\n");
			for(int i = 0;i < noOfLocations;i++)
				System.out.print("Location "+ i+ "\t" +forecast[i] + "\t" + (forecast[i] - sd[i]) + "\n");
