	...
	public static void main(String[] args) {
        ...
        for(int i = 1;i < noOfLocations;i++){
        	for(int j = 0;j < noOfDays * interval;j++){
        		...
        		for (int k=0; k<12; k++)
        		{
        			change = ThreadLocalRandom.current().nextDouble(10,50); //made modification from (5,30)
        		...
        		for (int k=12; k<24; k++)
        		{
        			change = ThreadLocalRandom.current().nextDouble(50, 100); //made modification from (20,50)
        			Act[k][j][i] = Act[k][j][0] + change;
        			Act[noOfSlots-k-1][j][i] = Act[noOfSlots-k-1][j][0] + change;
        		}
		...
		try {
			fwrite = new FileWriter("ActPre.txt");
        ...
        */
        	...
        	for(int j = 0;j < noOfLocations;j++)
        		{
        			...
        			FileWriter fwrite2;
        			try {
						fwrite2 = new FileWriter("SD.txt");
						PrintWriter p2 = new PrintWriter(fwrite2);
						p2.println(sd[i][j]);
        			} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
		...
		for(int i = 0;i < noOfLocations;i++){
			...
			{
				int k= ThreadLocalRandom.current().nextInt(0,15);
				forecast[j][i] = ThreadLocalRandom.current().nextDouble(Act[j][k][i] , Act[j][k][i]+5);
		...
		for(int i = 0;i < noOfLocations;i++){
			for (int j=0; j<noOfSlots;j++)
			{
				actual[j][i] = ThreadLocalRandom.current().nextDouble(forecast[j][i] - forecast[j][i] * .5 , forecast[j][i] + forecast[j][i] * .5);
			}
	...
	static void solveAndPrint(IloCplex cplex,double rcost[], double nrcost[],double[]forecast, double[] selectLocations, double[] nrGeneration, PrintWriter printWriter, PrintWriter solardata, PrintWriter nonrendata, PrintWriter costdata){
		...
		{			
			...
			if(cplex.solve()){
		        ...
		        for (int j = 0;j < nvars - 1;++j){
		        	...
		        	if (x[j]>0)
		        	{
		        			...
		        			
		        		{
		        			...
		        			nonren+=x[j]*bmva;
		        			totalnrcost+= x[j]*nrcost[j-colcount];
		        		}
		        	}
		        }
		        ...
		        solardata.println ((totalnrcost)+(totalrcost));
		        nonrendata.println ("renewable power  "+(demand -nonren)+"\t cost  "+(totalrcost));
		        costdata.println((totalrcost+totalnrcost));
